...
import codecs
import os
import Extensions.MLMSettings as MLMSettings

from SMLabel.libs.constants import DEFAULT_ENCODING
...
class YOLOWriter:
    ...
    def save(self, class_list=[], target_file=None):
        ...
        if target_file is None:
            ...
            #Shafi to investigate why class file got changed
            classes_file = MLMSettings.CLASS_FILE_DIRECORY #os.path.join(os.path.dirname(os.path.abspath(self.filename)), "classes.txt")
            out_class_file = open(classes_file, 'w')

        else:
            out_file = codecs.open(target_file, 'w', encoding=ENCODE_METHOD)
            #Shafi to investigate- why class file dir change
            classes_file = MLMSettings.CLASS_FILE_DIRECORY #os.path.join(os.path.dirname(os.path.abspath(target_file)), "classes.txt")
            out_class_file = open(classes_file, 'w')
...
class YoloReader:
    ...
    def __init__(self, file_path, image, class_list_path=None):
        ...
        if class_list_path is None:
            ...
            #Shafi to investigate- why the class text file changes
            self.class_list_path = MLMSettings.CLASS_FILE_DIRECORY #os.path.join(dir_path, "classes.txt")
        else:
